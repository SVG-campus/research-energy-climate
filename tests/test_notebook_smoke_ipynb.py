...
def test_ci_notebooks_config_lists_smoke_paths() -> None:
    ...
    assert "CHARTER_OPEN_METEO_STREAM_SMOKE.ipynb" in txt
...
def test_charter_extended_light_notebook_wired() -> None:
    p = Path(__file__).resolve().parents[1] / "notebooks" / "CHARTER_EXTENDED_LIGHT.ipynb"
    assert p.is_file(), f"missing {p}"
    nb = json.loads(p.read_text(encoding="utf-8"))
    blob_parts: list[str] = []
    for c in nb.get("cells", []):
        ...
        blob_parts.append(src)
    blob = "\n".join(blob_parts)
    assert "charter_extended_light_smoke" in blob and "assert_run_card" in blob
...
def test_charter_domain_stream_notebook_wired() -> None:
    p = Path(__file__).resolve().parents[1] / "notebooks" / "CHARTER_OPEN_METEO_STREAM_SMOKE.ipynb"
    assert p.is_file(), f"missing {p}"
    nb = json.loads(p.read_text(encoding="utf-8"))
    blob_parts: list[str] = []
    for c in nb.get("cells", []):
        ...
        blob_parts.append(src)
    blob = "\n".join(blob_parts)
    assert "charter_energy_open_meteo_stream_smoke" in blob and "load_dataset" in blob
